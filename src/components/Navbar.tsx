...
export const Navbar: React.FC<NavbarProps> = ({ currentTab, onSelectTab }) => {
  ...
  const allNavItems: { id: TabType; label: string; icon: React.ReactNode; badge?: number; badgeColor?: string }[] = [
  ...
  // Filter strictly according to the logged-in user's role:
  // - Warehouse sees only warehouse-related tabs (Warehouse, Issuance, GRN, Audit)
  // - Procurement sees only procurement-related tabs (Procurement, Vendors, GRN)
  // - Admin sees everything
  const navItems = allNavItems.filter(tab => isTabAllowed(currentUser?.role, tab.id));

  const handleLogoClick = () => {
    const target = getDefaultTabForRole(currentUser?.role);
    onSelectTab(target);
  };
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center space-x-3 cursor-pointer" onClick={handleLogoClick}>
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className={`text-[10px] uppercase px-2 py-0.5 rounded-full font-semibold border hidden sm:inline-block ${roleConfig.badgeBg}`}>
          ...
          <div className="flex items-center space-x-2.5">
            <div className="hidden lg:flex items-center bg-slate-900 border border-slate-800 rounded-full px-3.5 py-1.5 text-xs text-slate-300 gap-2">
              ...
              <span className="truncate max-w-[170px]">{roleConfig.scopeBadge}</span>
            </div>

            {lowStockCount > 0 && isTabAllowed(currentUser?.role, 'warehouse') && (
              <button
            ...
            {currentUser && (
              <div className="relative" ref={userMenuRef}>
                ...
                >
                  ...
                  <div className="hidden sm:block text-left">
                    <div className="text-xs font-semibold text-white leading-tight flex items-center space-x-1">
                      ...
                      <span className={`text-[9px] uppercase font-bold px-1.5 py-0.2 rounded border ${roleConfig.badgeBg}`}>
                        {roleConfig.shortTitle}
                      </span>
                    </div>
                  </div>
                ...
                {/* Dropdown Popover */}
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-2 w-80 rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl p-4 text-xs z-50 animate-in fade-in zoom-in-95">
                    {/* User Info Header */}
                    <div className="pb-3 border-b border-slate-800 flex items-center space-x-3">
                      <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center font-bold text-sm text-white uppercase shrink-0">
                        {currentUser.name.charAt(0)}
                      </div>
                      <div className="overflow-hidden flex-1">
                        <div className="font-bold text-white truncate">{currentUser.name}</div>
                        <div className="text-[11px] text-slate-400 truncate">{currentUser.email}</div>
                        <div className="text-[10px] text-indigo-400 mt-0.5 truncate">{currentUser.department}</div>
                      </div>
                    </div>

                    {/* Scope description badge */}
                    <div className="py-2.5 border-b border-slate-800">
                      <div className="text-[10px] uppercase tracking-wider font-semibold text-slate-400 mb-1">Active Permission Scope</div>
                      <div className="text-[11px] text-slate-300 bg-slate-950/80 p-2 rounded-lg border border-slate-800/80">
                        {roleConfig.description}
                      </div>
                    </div>

                    {/* Admin Portal Link - Only visible if admin */}
                    ...
                    <div className="py-2.5 border-b border-slate-800">
                      <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2 flex items-center space-x-1">
                        ...
                        <span>Switch Role Account (RBAC Demo)</span>
                      </div>
                      <div className="space-y-1">
                        {users.map(u => {
                          const uConfig = getRoleConfig(u.role);
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                switchUser(u.id);
                                if (!uConfig.allowedTabs.includes(currentTab)) {
                                  onSelectTab(uConfig.defaultTab);
                                }
                                setIsUserMenuOpen(false);
                              }}
                              className={`w-full flex items-center justify-between px-2.5 py-2 rounded-xl text-[11px] transition-colors cursor-pointer text-left ${
                                isActive
                            ...
                            >
                              <div className="truncate mr-2">
                                <div className="font-medium text-slate-200 truncate">{u.name}</div>
                                <div className="text-[10px] text-slate-400 truncate">{u.email}</div>
                              </div>
                              <span className={`text-[9px] uppercase px-1.5 py-0.5 rounded shrink-0 font-bold border ${uConfig.badgeBg}`}>
                                {u.role === 'admin' ? 'Admin' : u.role === 'warehouse_supervisor' ? 'WH Only' : u.role === 'procurement_manager' ? 'Proc Only' : u.role === 'qc_officer' ? 'QC Only' : 'Audit'}
                              </span>
                            </button>
                          );
                        })}
